#!/usr/bin/env python3
"""
Batch-learn relationship/couple/friend/human-thought topics from internet into local KB.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Learn relationship KB from web")
...
async def run() -> int:
    ...
    questions = build_relationship_seed_questions(args.count, args.seed)
    ...
    report["kb_path"] = str(settings.relationship_kb_path).replace("\\", "/")

    ts = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_json = out_dir / f"relationship_learn_report_{ts}.json"
    out_txt = out_dir / f"relationship_learn_report_{ts}.txt"
